...
__global__ void count_combinations(cuda_pre * pre,const uint32_t pre_count){//3 < N < 64
    ...
    if(idx < pre_count){
        ...
        uint64_t bh = pre[idx].bh; //block horizontal; count from bottom to top
        uint64_t bu = pre[idx].bu; //element 0 on the lower left
        uint64_t bd = pre[idx].bd; //element 0 on the lower right

        uint64_t cols[N] = {0};
        //the field on which the queens are set, split in columns
        //onehot coded for the row in which the queen is placed
        //0 means unset
        int64_t free_vectors[N] = {0};
        ...
        pre[idx].result = found_combinations;
    }
}
